...
if (-not $DryRun -and $allowStartMenuCleanup) {
    Invoke-TrackedOperation -Name 'Start Menu tile state' -Action 'Delete stale Start Menu tile state' -Scope 'CurrentUser' -Operation {
        ...
        if (Test-Path $startLayoutPath) {
            Get-ChildItem "$startLayoutPath\*windows.data.unifiedtile*" -EA 0 | Remove-Item -Recurse -Force -EA Stop
            Get-ChildItem "$startLayoutPath\*windows.data.taskmgr*" -EA 0 | Remove-Item -Recurse -Force -EA Stop
        }
    } | Out-Null
} elseif ($DryRun) {
    Invoke-TrackedOperation -Name 'Start Menu tile state' -Action 'Delete stale Start Menu tile state' -Scope 'CurrentUser' -Operation { } | Out-Null
