...
- Test mocks use Pester's `Should -Invoke` syntax; the 145-test suite passes under Pester 5.9 and 6.0.1 on PowerShell 7 and under Windows PowerShell 5.1 with Pester 5.9.
...
- Registry writes use a PowerShell 5.1-compatible typed property helper; generated revert scripts use `New-ItemProperty -PropertyType`.
...
- Compliance detection now requires manifest schema, catalog/config fingerprints, target scope, supported runtime, consistent operation accounting, and a complete registry stamp; it emits stable text or JSON reasons and scans past newer invalid manifests.
